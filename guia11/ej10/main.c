...
int main(void) {
	bagADT bag = newBag((int (*) (char *, char*)) strcasecmp);
    ...
    assert(deleteBag(bag, "carla") == 2);
    assert( size(bag) == 5 );
    assert(deleteBag(bag, "carla") == 1);
    assert( size(bag) == 5 );
    assert(deleteBag(bag, "carla") == 0);
    assert( size(bag) == 4 );
    assert(deleteBag(bag, "carla") == 0);


    assert(deleteBag(bag, "un desconocido") == 0);


    freeBag( bag );
    puts("OK!");

    bag = newBag((int (*) (char *, char*)) strcasecmp);
