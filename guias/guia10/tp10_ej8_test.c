...
TList listUnion(const TList l1, const TList l2) {
    ...
    if( l1 == NULL || (l2 != NULL && l2->elem < l1->elem) ) {
        new->elem = l2->elem;
        new->tail = listUnion(l1, l2->tail);
        return new;
    }

    // si llego aca ya se que l1 != NULL
    new->elem = l1->elem;
    if( l2 == NULL || l1->elem < l2->elem ) { 
        new->tail = listUnion(l1->tail, l2);
        return new;
    }
    new->tail = listUnion(l1->tail, l2->tail);
    return new;
}
